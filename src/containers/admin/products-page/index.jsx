...
import { productItems, productlist } from "@/utils/urls";
...
export default function ProductsPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = productItems.map((n) => n.Product);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - productItems.length) : 0;

  const visibleRows = React.useMemo(
    () =>
      stableSort(productItems, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
